...
from .utils import sample_uniform_points_in_unit_sphere, get_raster_points, check_voxels
...
class SurfacePointCloud:
    """
    Sampled point cloud on mesh surface.
    Points stored in KD-Tree, for querying closest point when computing SDF.
    """
    ...
    def get_sdf_in_batches(self, query_points, use_depth_buffer=False, sample_count=11, batch_size=1000000, return_gradients=False):
        ...
        batches = [
            ...
            for points in tqdm(np.array_split(query_points, n_batches), desc="Get SDF in Batches")
        ]
        if return_gradients:
    ...
    def get_voxels(self, volume_size, voxel_resolution, use_depth_buffer=False, sample_count=11, pad=False, check_result=False, return_gradients=False):

        result = self.get_sdf_in_batches(get_raster_points(volume_size, voxel_resolution), use_depth_buffer, sample_count, return_gradients=return_gradients)
        
        if not return_gradients:
            sdf = result
        else:
            sdf, gradients = result
            voxel_gradients = np.reshape(gradients, (volume_size[0], volume_size[1], volume_size[2], 3))
        
        voxels = sdf.reshape((volume_size[0], volume_size[1], volume_size[2]))
